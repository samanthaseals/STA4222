...
estvar = sum(data$varhat_ybar) # find the variance of ybar
...
estvar = sum(data$varhat_tauhat) # find the variance of ybar
SE = sqrt(estvar) # find the SE
n = sum(data$n_h) # find overall sample size
H = nrow(data) # count the number of rows for number of strata
t = qt(.975, n-H)
...
```{r, echo = TRUE}
# set values to use in computation:
p = sum(data$phat) # point estimate for tau
estvar = sum(data$varhat_phat) # find the variance of ybar
SE = sqrt(estvar) # find the SE
